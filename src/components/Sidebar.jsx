...
export default function Sidebar() {
  ...
  return (
    <aside style={{ width: 200, minHeight: "100vh", background: "#0F172A", display: "flex", flexDirection: "column", position: "fixed", left: 0, top: 0, bottom: 0, zIndex: 50 }}>
      ...
      <nav style={{ flex: 1, padding: "12px 8px" }}>
        {NAV.filter(item => !item.adminOnly || user?.role === "admin").map(item => {
          const active = location.pathname === item.path || (item.path !== "/dashboard" && location.pathname.startsWith(item.path) && item.path !== "/shipments/create" ? location.pathname === item.path : false);
